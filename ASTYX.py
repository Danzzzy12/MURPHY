...
class LOGE:
    ...
    def logo_ku(self):
      ...
      \rAuthor : [bold green] MURPHYZZXX[/]  Version : [bold green]8.0[/]"""))
class Load:
...
class VALIDATE:
    ...
    def ua_fb(self):
        andro_version = random.choice(["8.0", "8.1", "9", "10", "11", "12", "13", "7.0", "7.1.1", "6.0.1", "5.1.1", "5.0.2"])
        andro_build = random.choice([
            "SM-G975F", "SM-A505F", "SM-G960F", "RMX2020", "RMX2195", "RMX3085", "A501CG", "ASUS_Z01RD", "POCO F1",
            "Mi A1", "Redmi Note 7"
        ])
        android_id = "".join(random.choices("0123456789abcdef", k=16))
        facebook_version = f"{random.randint(300, 500)}.{random.randint(0, 9)}.{random.randint(0, 9)}.{random.randint(1, 99)}.{random.randint(10, 200)}"
        fbbv = str(random.randint(70000000, 99999999))
        fbrv = str(random.randint(1000000000, 2147483647))
        density = f"{random.randint(100, 480)/100:.2f}"
        width = str(random.randint(360, 1440))
        height = str(random.randint(640, 3200))
        locale = random.choice(["en_US", "id_ID", "ru_RU", "it_IT", "es_ES", "fr_FR", "de_DE", "ja_JP", "ko_KR", "zh_CN", "pt_BR", "ar_AR", "vi_VN", "th_TH", "tr_TR"])
        carrier = random.choice(["Telkomsel", "XL Axiata", "Indosat", "Indosat Ooredoo"])
        architecture = random.choice(["arm64-v8a", "armeabi-v7a", "x86", "armeabi"])
        build_id = random.choice(['RP1A', 'QP1A', 'TP1A', 'SP1A','PQ3B','RQ1A','UP1A','AP1A', 'PPR1', 'REL'])
        build_number = f"{build_id}.{random.randint(190805, 231027)}.{random.randint(1,99)}"
        os_version = random.choice(["10", "11", "12", "13", "14"])
        user_agents = [
        f"Dalvik/2.1.0 (Linux; U; Android {andro_version}; {andro_build} Build/{build_number}) [FBAN/FB4A;FBAV/{facebook_version};FBBV/{fbbv};FBDM/{{density={density},width={width},height={height}}};FBLC/{locale};FBRV/{fbrv};FBCR/{carrier};FBMF/{random.choice(['samsung', 'realme', 'xiaomi', 'oppo', 'vivo', 'google', 'oneplus', 'huawei','motorola','asus'])};FBBD/{andro_build};FBPN/com.facebook.katana;FBDV/{andro_build};FBSV/{andro_version};FBOP/1;FBCA/{architecture}:;]",
        f"[FBAN/FB4A;FBAV/{facebook_version};FBBV/{fbbv};FBDM/{{density={density},width={width},height={height}}};FBLC/{locale};FBRV/{fbrv};FBCR/{carrier};FBMF/{random.choice(['samsung', 'realme', 'xiaomi', 'oppo', 'vivo', 'google', 'oneplus', 'huawei','motorola','asus'])};FBBD/{andro_build};FBPN/com.facebook.katana;FBDV/{andro_build};FBSV/{andro_version};FBOP/1;FBCA/{architecture}:;]",
        f"[FBAN/MessengerLite;FBAV/{facebook_version};FBBV/{fbbv};FBDM/{{density={density},width={width},height={height}}};FBLC/{locale};FBRV/{fbrv};FBCR/{carrier};FBMF/{random.choice(['samsung', 'realme', 'xiaomi', 'oppo', 'vivo', 'google', 'oneplus', 'huawei','motorola','asus'])};FBBD/{andro_build};FBPN/com.facebook.mlite;FBDV/{andro_build};FBSV/{andro_version};FBOP/1;FBCA/{architecture}:;]",
        f"[FBAN/Orca-Android;FBAV/{facebook_version};FBBV/{fbbv};FBDM/{{density={density},width={width},height={height}}};FBLC/{locale};FBRV/{fbrv};FBCR/{carrier};FBMF/{random.choice(['samsung', 'realme', 'xiaomi', 'oppo', 'vivo', 'google', 'oneplus', 'huawei','motorola','asus'])};FBBD/{andro_build};FBPN/com.facebook.orca;FBDV/{andro_build};FBSV/{andro_version};FBOP/5;FBCA/{architecture}:;]", #Facebook Lite
        ]
